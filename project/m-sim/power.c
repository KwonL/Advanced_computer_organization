...
void update_power_stats()
{
  ...
  if(rename_access) {
    ...
    rename_power_cc2+=((double)rename_access/(double)decode_width)*power.rename_power;
    rename_power_cc3+=((double)rename_access/(double)decode_width)*power.rename_power;
  }
  else 
  ...
  if(window_preg_access) {
    if(window_preg_access <= 3*issue_width)
      ...
      window_power_cc1+=((double)window_preg_access/(3.0*(double)issue_width))*power.rs_power;
    window_power_cc2+=((double)window_preg_access/(3.0*(double)issue_width))*power.rs_power;
    window_power_cc3+=((double)window_preg_access/(3.0*(double)issue_width))*power.rs_power;
  }
  else
  ...
  if(window_preg_access) {
    if(window_preg_access <= 3*issue_width)
      ...
      window_power_cc1+=((double)window_preg_access/(3.0*(double)issue_width))*(power.rs_power_nobit + window_af_b*power.rs_bitline);
    window_power_cc2+=((double)window_preg_access/(3.0*(double)issue_width))*(power.rs_power_nobit + window_af_b*power.rs_bitline);
    window_power_cc3+=((double)window_preg_access/(3.0*(double)issue_width))*(power.rs_power_nobit + window_af_b*power.rs_bitline);
  }
  else
    window_power_cc3+=turnoff_factor*power.rs_power;
#else
  ...
  if(window_selection_access) {
    if(window_selection_access <= issue_width)
      ...
      window_power_cc1+=((double)window_selection_access/((double)issue_width))*power.selection;
    window_power_cc2+=((double)window_selection_access/((double)issue_width))*power.selection;
    window_power_cc3+=((double)window_selection_access/((double)issue_width))*power.selection;
  }
  else
  ...
  if(window_wakeup_access) {
    if(window_wakeup_access <= issue_width)
      ...
      window_power_cc1+=((double)window_wakeup_access/((double)issue_width))*power.wakeup_power;
    window_power_cc2+=((double)window_wakeup_access/((double)issue_width))*power.wakeup_power;
    window_power_cc3+=((double)window_wakeup_access/((double)issue_width))*power.wakeup_power;
  }
  else
  ...
  if(regfile_access) {
    if(regfile_access <= (3.0*commit_width))
      ...
      regfile_power_cc1+=((double)regfile_access/(3.0*(double)commit_width))*power.regfile_power;
    regfile_power_cc2+=((double)regfile_access/(3.0*(double)commit_width))*power.regfile_power;
    regfile_power_cc3+=((double)regfile_access/(3.0*(double)commit_width))*power.regfile_power;
  }
  else
  ...
  if(regfile_access) {
    if(regfile_access <= (3.0*commit_width))
      ...
      regfile_power_cc1+=((double)regfile_access/(3.0*(double)commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
    regfile_power_cc2+=((double)regfile_access/(3.0*(double)commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
    regfile_power_cc3+=((double)regfile_access/(3.0*(double)commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
  }
  else
    regfile_power_cc3+=turnoff_factor*power.regfile_power;
#endif
  ...
  if(resultbus_access) {
    ...
    if(resultbus_access <= issue_width) {
      ...
      resultbus_power_cc1+=((double)resultbus_access/(double)issue_width)*power.resultbus;
    }
    resultbus_power_cc2+=((double)resultbus_access/(double)issue_width)*power.resultbus;
    resultbus_power_cc3+=((double)resultbus_access/(double)issue_width)*power.resultbus;
  }
  else
  ...
  if(resultbus_access) {
    ...
    if(resultbus_access <= issue_width) {
      ...
      resultbus_power_cc1+=((double)resultbus_access/(double)issue_width)*resultbus_af_b*power.resultbus;
    }
    resultbus_power_cc2+=((double)resultbus_access/(double)issue_width)*resultbus_af_b*power.resultbus;
    resultbus_power_cc3+=((double)resultbus_access/(double)issue_width)*resultbus_af_b*power.resultbus;
  }
  else
    resultbus_power_cc3+=turnoff_factor*power.resultbus;
#endif
...
double dcl_compare_power(int compare_bits)
{
  ...
  num_comparators = (decode_width - 1) * (decode_width);
...
double total_clockpower(double die_length)
{
  ...
  /* pipe stage 0/1 */
  num_piperegs = issue_width*inst_length + data_width;
  /* pipe stage 1/2 */
  num_piperegs += issue_width*(inst_length + 3 * ROB_size);
  /* pipe stage 2/3 */
  num_piperegs += issue_width*(inst_length + 3 * ROB_size);
  /* pipe stage 3/4 */
...
double compute_resultbus_power()
{
  ...
  regfile_height = rf_size * (RegCellHeight + 
			       WordlineSpacing * 3 * issue_width); 
  ...
  Ctotal += 2.0 * (data_width + npreg_width) * (issue_width)* Cline;
     ...
     power_result_type *power;
{
  ...
  npreg_width = (int)ceil(logtwo((double)ROB_size));
  ...
  predeclength = MD_NUM_IREGS * 
    (RatCellHeight + 3 * decode_width * WordlineSpacing);

  wordlinelength = npreg_width * 
    (RatCellWidth + 
     6 * decode_width * BitlineSpacing + 
     RatShiftRegWidth*RatNumShift);

  bitlinelength = MD_NUM_IREGS * (RatCellHeight + 3 * decode_width * WordlineSpacing);
