...
static counter_t ROB_count;		/* cumulative ROB occupancy */
static counter_t ROB_fcount;		/* cumulative ROB full count */
...
/*
 * Prints both the integer and floating point physical register file states
 */
static void print_rf(){
  int i;
  for(i=0; i<rf_size; i++){
    printf("$r%d = %d\n", i, int_reg_file[i].state);
  }

  printf("\n");

  for(i=0; i<rf_size; i++){
    printf("$f%d = %d\n", i, fp_reg_file[i].state);
  }
}
...
static void print_rename_tables(){
  ...
  for(cid = 0; cid < num_contexts; cid++){
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (ifq_size < 1 || (ifq_size & (ifq_size - 1)) != 0)
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  for(i = 0; i < num_contexts; i++)
    if (contexts[i].pred)
      bpred_reg_stats(contexts[i].pred, sdb);
...
struct ROB_entry {
  ...
  /* input dependent links, the output chains rooted above use these
     ...
     been set non-zero, the ROB operation has all of its register
...
#define OPERANDS_NONREADY(RS)                                              \
  (2 - ((RS)->idep_ready[0] + (RS)->idep_ready[1]))
...
rob_init(int context_id)
{
  ...
  /* shared statistics among threads...change later? */
  ROB_count = 0;
  ROB_fcount = 0;
}
...
/*
 * the create vector maps a logical register to a creator in the ROB (and
...
#define CREATE_VECTOR(N)        (BITMAP_SET_P(contexts[bitmap_context_id].use_spec_cv, CV_BMAP_SZ, (N))\
				 ? contexts[bitmap_context_id].spec_create_vector[N]                \
				 : contexts[bitmap_context_id].create_vector[N])

/* read a create vector timestamp entry */
#define CREATE_VECTOR_RT(N)     (BITMAP_SET_P(contexts[bitmap_context_id].use_spec_cv, CV_BMAP_SZ, (N))\
				 ? contexts[bitmap_context_id].spec_create_vector_rt[N]             \
				 : contexts[bitmap_context_id].create_vector_rt[N])

/* set a create vector entry */
#define SET_CREATE_VECTOR(N, L) (spec_mode                              \
				 ? (assert(bitmap_context_id == L.rs->context_id),\
                                    BITMAP_SET(contexts[bitmap_context_id].use_spec_cv, CV_BMAP_SZ, (N)),\
				    contexts[bitmap_context_id].spec_create_vector[N] = (L))        \
				 : (assert(bitmap_context_id == L.rs->context_id),\
				    contexts[bitmap_context_id].create_vector[N] = (L)))
...
cv_init(void)
{
  ...
  for(k=0; k < num_contexts;k++){
    ...
    for (i=0; i < MD_TOTAL_REGS; i++)
      {
	contexts[k].create_vector[i] = CVLINK_NULL;
	contexts[k].create_vector_rt[i] = 0;
	contexts[k].spec_create_vector[i] = CVLINK_NULL;
	contexts[k].spec_create_vector_rt[i] = 0;
      }
    /* all create vector entries are non-speculative */
    BITMAP_CLEAR_MAP(contexts[k].use_spec_cv, CV_BMAP_SZ);
  }
}
...
cv_dump(FILE *stream, int context_id)				/* output stream */
{
  ...
  for (i=0; i < MD_TOTAL_REGS; i++)
	...
	fprintf(stream, "[cv%02d]: from %s, idx: %d\n",
		i, (ent.rs->in_LSQ ? "LSQ" : "ROB"),
		(int)(ent.rs - (ent.rs->in_LSQ ? contexts[context_id].LSQ : contexts[context_id].ROB)));
    }
}
  ...
  while (contexts[context_id].ROB_num > 0 && committed < commit_width)
    {
      ...
      if (!rs->completed)
	{
	  /* at least ROB entry must be complete */
	  break;
	}
...
{
  int i, ROB_index = contexts[context_id].ROB_tail, LSQ_index = contexts[context_id].LSQ_tail;
  ...
  while (ROB_index != branch_index)
      ...
      if (contexts[context_id].ROB[ROB_index].ea_comp)
	{
	  ...
	  for (i=0; i<MAX_ODEPS; i++)
	    {
	      RSLINK_FREE_LIST(contexts[context_id].LSQ[LSQ_index].odep_list[i]);
      ...
      for (i=0; i<MAX_ODEPS; i++)
	{
	  RSLINK_FREE_LIST(contexts[context_id].ROB[ROB_index].odep_list[i]);
	  /* blow away the consuming op list */
	  contexts[context_id].ROB[ROB_index].odep_list[i] = NULL;
	}
  ...
  {
    /* clear the speculative create vector for the context */
    int i;
    for(i=0;i<MD_TOTAL_REGS;i++){
...
writeback(void)
{
  ...
  while ((rs = eventq_next_event()))
      ...
      if (rs->recover_inst)
	{
	  ...
	  contexts[rs->context_id].fetch_issue_delay = branch_penalty;
      ...
      for (i=0; i<MAX_ODEPS; i++)
	{
	  if (rs->onames[i] != NA)
	      ...
	      if (rs->spec_mode)
		{
		  ...
		  link = contexts[rs->context_id].spec_create_vector[rs->onames[i]];
		  if (/* !NULL */link.rs
		      && /* refs RS */(link.rs == rs ))//&& link.odep_num == i))
		    {
		      /* the result can now be read from a physical register,
			 indicate this as so */
		      contexts[rs->context_id].spec_create_vector[rs->onames[i]] = CVLINK_NULL;
		      contexts[rs->context_id].spec_create_vector_rt[rs->onames[i]] = sim_cycle;
		    }
	      ...
	      else
		{
		  /* update the non-speculative create vector, future
		     ...
		     reg file */
		  link = contexts[rs->context_id].create_vector[rs->onames[i]];
		  if (/* !NULL */link.rs
		      && /* refs RS */(link.rs == rs))// && link.odep_num == i))
		    {
		      /* the result can now be read from a physical register,
			 indicate this as so */
		      contexts[rs->context_id].create_vector[rs->onames[i]] = CVLINK_NULL;
		      contexts[rs->context_id].create_vector_rt[rs->onames[i]] = sim_cycle;
		    }
		  /* else, creator invalidated or there is another creator */
		}
		  ...
		  if (RSLINK_VALID(olink))
		      ...
		      if (OPERANDS_READY(olink->rs))
			{
			  ...
			  if (!olink->rs->in_LSQ
			    ...
			    //another instruction wokenup -> ready
			  }
...
/*
 *  ISSUE() - issue instructions to functional units
 */
	      ...
	      int idep_name)			/* input register name */
{
  ...
  int bitmap_context_id = rs->context_id;
  ...
  if(head.rs->context_id != rs->context_id){
    int i;
    for(i=0;i<MD_TOTAL_REGS;i++){
      if(contexts[rs->context_id].create_vector[idep_name].rs)
	assert(contexts[rs->context_id].create_vector[idep_name].rs->context_id == bitmap_context_id);
      if(contexts[rs->context_id].spec_create_vector[idep_name].rs)
	assert(contexts[rs->context_id].spec_create_vector[idep_name].rs->context_id == bitmap_context_id);
    }

    printf("%d \n",BITMAP_SET_P(contexts[bitmap_context_id].use_spec_cv, CV_BMAP_SZ, (idep_name)));
    if(BITMAP_SET_P(contexts[bitmap_context_id].use_spec_cv, CV_BMAP_SZ, (idep_name))){
      assert(head.rs == contexts[rs->context_id].spec_create_vector[idep_name].rs);
    }else{
      assert(head.rs == contexts[rs->context_id].create_vector[idep_name].rs);
    }
    assert(FALSE);
    //exit(-1);
  }
...
static INLINE void
rob_install_odep(struct ROB_entry *rs,	        /* creating ROB entry */
		 ...
		 int odep_name)			/* output register name */
{
  ...
  //assertions:
    ...
    for(k=0;k<num_contexts;k++)
    for(i=0;i<MD_TOTAL_REGS;i++){
      if(contexts[k].create_vector[i].rs)
	assert(contexts[k].create_vector[i].rs->context_id == k);
      if(contexts[k].spec_create_vector[i].rs)
	assert(contexts[k].spec_create_vector[i].rs->context_id == k);
    }
  }
}
...
/* the last operation that dispatch() attempted to dispatch, for
...
/*
 * TODO: clean this up - it is a bit sloppy in places
 */
static void
dispatch(void)
{
...
#include "machine.def"
	      ...
	      && target_PC != contexts[disp_context_id].pred_PC && br_pred_taken))
	{
	  ...
	  if (!pred_perfect)
	    contexts[disp_context_id].fetch_issue_delay = branch_penalty;
      ...
      if (!contexts[disp_context_id].spec_mode)
	{
	  ...
	  if (contexts[disp_context_id].pred_PC != regs->regs_NPC)// && !fetch_redirected[disp_context_id])
	    {
...
fetch(int* current_fetch_context_ptr)
{
  ...
  for (i=0;
       ...
       && contexts[current_fetch_context].fetch_num < ifq_size;
...
"\n"
    ...
    {
      /* dump event queue contents */
      cv_dump(stream, regs->context_id);
    }
...
#include "machine.def"
  ...
  for (;;)
	...
	for(i=0;i<num_contexts;i++){
	  ...
	  IFQ_fcount += ((contexts[i].fetch_num == ifq_size) ? 1 : 0);
	  ROB_count += contexts[i].ROB_num;
	  ROB_fcount += ((contexts[i].ROB_num == ROB_size) ? 1 : 0);
...
void smt_print_stats(){
  ...
  for(i=0;i<num_contexts;i++){
    fprintf(stderr, "IPC %d: %1.4f\n",i,contexts[i].sim_num_insn / (float)sim_cycle);
  }
  fprintf(stderr, "THROUGHPUT IPC: %1.4f\n",sim_num_insn / (float)sim_cycle);
