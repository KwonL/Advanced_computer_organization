...
stat_eval_ident(struct eval_state_t *es)/* an expression evaluator */
{
  ...
  switch (stat->sc)
	...
	struct eval_state_t *es = eval_new(stat_eval_ident, sdb, my_mem);
	char *endp;
...
stat_new(void)
{
  ...
  sdb->stats = NULL;
  sdb->evaluator = eval_new(stat_eval_ident, sdb, NULL);
		...
		FILE *fd)		/* output stream */
{
  ...
  switch (stat->sc)
    ...
    case sc_formula:
      {
	/* instantiate a new evaluator to avoid recursion problems */
	struct eval_state_t *es = eval_new(stat_eval_ident, sdb, NULL);
	char *endp;
